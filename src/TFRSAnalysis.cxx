...
Bool_t TFRSAnalysis::InitEventClasses()
{
  ...
  for (int i = 0; i < 8; i++)
  {
    ...
    if (fMUSICPar->exclude_de1_adc_channel[i])
    { // true means exclude
    ...
    if (fMUSICPar->exclude_de2_adc_channel[i])
    { // true means exclude
    ...
    if (fMUSICPar->exclude_de3_adc_channel[i])
    { // true means exclude
    ...
    if (fMUSICPar->exclude_de4_adc_channel[i])
    { // true means exclude
  ...
  for (int igate = 0; igate < 15; igate++)
  {
    ...
    if (0 <= igate && igate < 5)
    {
      sprintf(gate_name, "cID_ZAoQ(%d)", igate);
    }
    if (5 <= igate && igate < 10)
    {
      sprintf(gate_name, "cID_x2AoQ(%d)", igate - 5);
    }
    if (10 <= igate && igate < 15)
    {
      sprintf(gate_name, "cID_x4AoQ(%d)", igate - 10);
    }
...
Bool_t TFRSAnalysis::InitStandartFRSParameters()
{
...
Bool_t TFRSAnalysis::InitFOOTParameters()
{
  ...
  std::string nameSetupFile("setup/FOOT_");
  nameSetupFile += nameExperiment;
  nameSetupFile += ".C";
...
TFRSAnalysis::~TFRSAnalysis()
{
  ...
  // std::cout << "**** TFRSAnalysis: Delete ****" << endl;
}
...
Int_t TFRSAnalysis::UserPreLoop()
{
  ...
  if (IsBatchMode())
    printf("go4 found in batch mode\n");
  if (IsClientMode())
    printf("go4 found in client mode\n");
  if (IsServerMode())
    printf("go4 found in server mode\n");
...
Int_t TFRSAnalysis::UserEventFunc()
{
  ...
  if (fEvents == 1)
    std::cout << "First event: " << count << std::endl;
  fLastEvent = count;
